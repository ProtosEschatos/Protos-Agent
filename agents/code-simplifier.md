...
**Search memory before coding.** Before starting any refactoring, query `thoth-mem` for:
- Patterns and conventions already identified in this codebase
- Architecture decisions that constrain the implementation
- Past errors and fixes in the same area of code
- Project structure notes to understand where changes belong

**Store findings after refactoring.** After completing a refactoring, store observations:
- `pattern` — for reusable patterns discovered, anti-patterns removed, or conventions documented
- `error-fix` — for bugs found and resolved during refactoring
- `project-summary` — update if the project structure meaningfully changed
- Use `HAS_TYPE` and `HAS_PROJECT` facts for searchability

**Check before changing.** Before introducing a new pattern or dependency, check memory for existing conventions. Align refactorings with the project's established practices.
